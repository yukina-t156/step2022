                ...
                std::map<std::string, std::pair<std::string, std::string> > &route, long long &visited_num)
{
    ...
    ofs << visited_num << " pages visited and via " << route_st.size() << " pages. (DFS)" << std::endl; //訪ねたページ数を表示
    //経路の出力開始
...
//    std::cout << start.second << " " << start.first << std::endl; //開始地点の出力
    ...
    std::map<std::string, bool> checked; // 訪れた場所を記録するmap
    checked[start.first] = true;

    std::stack<std::pair<std::string, std::string> > st; //スタックを宣言
    st.push(start); // 開始地点をプッシュ
    while (st.size() != 0){
        visit_counter++; //カウントしておく
        auto now = st.top(); // スタックの一番上のペアをnowに取り出す
        st.pop(); // stから削除
        ...
        checked[now.first] = true; // 探索済みにする
    ...
    if (goal.first != "-1"){
        ...
        int show = 0;
        std::cin >> show;
        if (show) show_route(start, goal, route, visit_counter);
    }else{
        //見つからなかった場合
        std::cout << "Not found: " << goal.second << std::endl;
        std::cout << "("<< visit_counter<< " pages visited)" << std::endl; //訪ねたページ数を表示
